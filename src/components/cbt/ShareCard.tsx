...
export const ShareCard = forwardRef<HTMLDivElement, ShareCardProps>(({ sessionData }, ref) => {
  const emotion = sessionData.selectedEmotion ? EMOTIONS.find((e) => e.id === sessionData.selectedEmotion) : null;
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-foreground">{sessionData.customEmotion || emotion?.label || "未命名"}</p>
            <p className="text-xs text-muted-foreground">强度：{sessionData.emotionIntensity}/10</p>
          </div>
        </div>
        {sessionData.bodySensation && (
          <p className="text-xs text-muted-foreground mt-2 italic">身体感受：{sessionData.bodySensation}</p>
        )}
      </motion.div>
        ...
        {distortions.length > 0 && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1">
              {distortions.map((d) => (
                <span key={d!.id} className="px-2 py-0.5 bg-sky/30 text-sky-foreground text-xs rounded-full">
                  {d!.name}
                </span>
              ))}
            </div>
      ...
      >
        <p className="text-sm text-lavender font-medium mb-1">今天，我照顾了自己的情绪 💜</p>
        <p className="text-xs text-muted-foreground">MindFlow AI</p>
      </motion.div>
    </div>
  );
});
